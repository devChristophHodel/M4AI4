...
```
```{r}
print("En la visualización del elbow method encima, a partir de cinco clusters, la reducción en la suma total de cuadrados internos parece
estabilizarse, indicando que K = 5. Parece una buena elección.")
```
...
```
```{r}
print("El numero optimo de clusters segun el dendrograma parece ser 5. Al cortar el dendrogrma a ese altura, las uniones entre los grupos todavia occuren a una altura relativamente eleveada, lo que indica que la distancia intercluster es significativa. Ademas elegir 5 clusters es coherente con lo observado en el metodo del code (ejercicio 1).")
```
...
aggregate(vinos_df_orig$quality, by = list(Cluster = km_clusters_vino_e_4$cluster), FUN = mean)
...
aggregate(vinos_df_orig$quality, by = list(Cluster = km_clusters_vino_6$cluster), FUN = mean)
